...
type SaveSlot = {
  id: string;
  profile: Profile;
  state: any; // persisted game state snapshot
  createdAt: number;
  name?: string;
};
...
export default function LoadGameScreen({ navigation }: Props) {
  ...
  const writeSaves = async (arr: SaveSlot[]) => {
  ...
  const migrateIfNeeded = async () => {
    try {
      ...
      if (existing) return; // already using new schema
      ...
      const slot: SaveSlot = {
        ...
        state,
        createdAt: Date.now(),
        name: state.profile ? `${state.profile.firstName} ${state.profile.lastName}` : 'Save 1',
      };
  ...
  const doLoad = async (slot: SaveSlot) => {
    // Overwrite current store with snapshot
    try {
      // zustand exposes setState on the hook function
      (useGameStore as any).setState(slot.state ?? {});
      navigation.replace('Game', { profile: slot.profile });
    } catch (e) {
  ...
  const renderSlot = ({ item }: { item: SaveSlot }) => {
    const avatar = characters[item.profile?.avatar ?? 0] ?? characters[0];
...
const styles = StyleSheet.create({
  ...
  loadBtn: { padding: 8, marginRight: 8, backgroundColor: '#e6f0ff', borderRadius: 6 },
  delBtn: { padding: 8 },
  avatar: { width: 56, height: 56, borderRadius: 6 },
